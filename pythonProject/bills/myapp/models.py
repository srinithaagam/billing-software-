...
#     def save(self, *args, **kwargs):
#         if not self.bill_number:
#             last = Bill.objects.order_by('id').last()
...
class Bill(models.Model):
    bill_number = models.CharField(max_length=6, unique=True, blank=True)
    customer_name = models.CharField(max_length=100)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def save(self, *args, **kwargs):
        if not self.bill_number:
            while True:
                ...
                if not Bill.objects.filter(bill_number=random_number).exists():
                    self.bill_number = random_number
                    break
        super().save(*args, **kwargs)
